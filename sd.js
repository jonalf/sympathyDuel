...
function init() {
	...
	rc.setAttribute("src", "images/cards/" + 
	...
	bc.setAttribute("src","images/cards/" + 
...
function selectCard( card ) {
	...
	if ( discarding ) {
	    $("img[src='images/cards/" + card + ".png']").remove();
	    ...
	    $(".selected").removeClass("selected");
	    selected = card;
	    $("img[src='images/cards/" + card + ".png']").addClass("selected");
	}
    }
}
...
function convert( card ) {
	...
	if ( (Math.abs( candleRank - mindRank ) == 1 ||
	    ...
	    //change view
	    var c = $("img[src='images/cards/" + card + ".png']");
	    c.attr("src", "images/cards/" + selected + ".png");
...
function resizeMind( player ) {
	...
	var nc = document.createElement("img");
	nc.setAttribute("class", cardClass);
	nc.setAttribute("src", "images/cards/"+mind[player][i]+".png");
	nc.setAttribute("onclick", "selectCard(\""
			+mind[player][i]+"\")");
	m.append(nc);
    }
